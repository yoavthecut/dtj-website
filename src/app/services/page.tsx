...
function ComingSoonBadge() {
  return (
    <div className="mt-auto flex flex-col gap-3 pt-4 border-t border-white/15">
      <div className="inline-flex items-center justify-center gap-2 bg-brand-gold/15 border border-brand-gold/50 text-brand-gold font-semibold px-5 py-3 rounded-full text-sm tracking-wide self-start">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        Coming Soon
      </div>
      <p className="text-white/65 text-xs">Online booking opens soon. In the meantime, reach out and we&apos;ll get you started.</p>
    </div>
  );
}
...
export default function ServicesPage() {
  return (
    <div className="flex flex-col overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={reveal}
              className="bg-[#6B21A8] border border-purple-700/60 rounded-2xl p-8 flex flex-col gap-5 shadow-lg text-white">
              <div className="w-12 h-12 rounded-xl bg-brand-gold/15 flex items-center justify-center text-brand-gold">
                <ConsultationIcon />
              </div>
              <h3 className="font-serif text-xl font-bold text-white leading-snug">1:1 Consultation</h3>
              <p className="text-xs text-brand-gold/80 font-semibold uppercase tracking-wider">45 min</p>
              <p className="text-white text-sm font-semibold">Want to begin the conversion process but don&apos;t know where to start?</p>
              <ul className="flex flex-col gap-2">
                {[
                  ...
                  <li key={b} className="flex items-start gap-2 text-white/80 text-sm leading-relaxed">
                    <span className="mt-[6px] shrink-0 w-1.5 h-1.5 rounded-full bg-brand-gold" />
                    {b}
                  </li>
                ))}
              </ul>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={reveal}
              ...
              <h3 className="font-serif text-xl font-bold text-white leading-snug">Emotional Support</h3>
              <p className="text-xs text-brand-gold/80 font-semibold uppercase tracking-wider">45 min</p>
              <p className="text-white text-sm font-semibold">A safe, non-judgmental space to discuss the personal and emotional aspects of conversion:</p>
              <ul className="flex flex-col gap-2">
                {[
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={2} variants={reveal}
              ...
              <h3 className="font-serif text-xl font-bold text-white leading-snug">Hebrew Classes</h3>
              <p className="text-xs text-brand-gold/80 font-semibold uppercase tracking-wider">45 min · Beginner Level</p>
              <p className="text-white text-sm font-semibold">Our beginner program is designed to take you from the very first steps of the Aleph-Bet to a confident understanding of Hebrew basics, specifically tailored for the conversion journey.</p>
              <p className="text-white/90 text-sm font-semibold">What you will gain:</p>
              <ul className="flex flex-col gap-2">
                {[
      ...
      <section className="relative bg-[#BAE6FD] pt-16 pb-36 px-0 overflow-hidden">
        <style>{`
        ...
        <div className="max-w-6xl mx-auto px-6 mb-12">
          <motion.p initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={reveal}
            className="text-brand-purple text-xs font-bold tracking-[0.25em] uppercase text-center mb-3">
            What people say
          </motion.p>
          <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={reveal}
        ...
        >
          <div className="marquee-services-track flex gap-5 w-max px-5">
            {[...testimonials, ...testimonials].map((t, i) => (
              <div key={i} className="w-80 shrink-0 bg-white border border-gray-200 hover:border-brand-purple/40 rounded-2xl p-7 flex flex-col gap-4 transition-colors duration-300 shadow-sm hover:shadow-md">
      ...
      <section className="bg-[#fb7109] py-20 px-6">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={reveal}
          ...
          <div>
            ...
            <p className="text-white/80 text-sm max-w-md">Online booking is launching soon. Reach out now and we&apos;ll guide you to the right session.</p>
          </div>
          <Link href="/contact"
